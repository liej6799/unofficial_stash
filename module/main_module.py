...
from .common.common_config import get_json_base_path, get_api_status_base_path
from .helpers.json import read_file, dumps_class_to_str
from .helpers.language import load_language
from .helpers.network import post_data_with_bearer
from .models.database_model import Goals, Securities
from .models.main_model import dashboard_model, etf_detail_model, securities_model
    ...
    if dashboard_result is not None:
        try:
            ...
            if date.today() > latest_goals_date:
                ...
                click.echo(click.style(load_language().t('lang.LATEST_DASHBOARD'), fg='green'))
        except KeyError:
...
def etf_detail_module():
    ...
    if date.today() > latest_securities_date:
                                ...
                                if securities['units'] is not None:
                                    if securities['units'] != 0.0:
                                        all_securities.append(
                                            securities_model(securities['symbol'], securities['units'],
        ...
        # Instead of read new data, read previous one
        click.echo(click.style(load_language().t('lang.LATEST_ETF'), fg='green'))
